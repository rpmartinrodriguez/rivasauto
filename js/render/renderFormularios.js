...
window.imprimirBoletoHtml = (data) => {
  // Aseguramos de que el logo global (de estar en el DOM) no moleste
  const globalLogo = document.getElementById('print-logo');
  if(globalLogo) globalLogo.classList.add('hidden');

  // Insertamos el logo posicionado en la esquina superior derecha
  let printHtml = `
  ...
  if(data.tipo === 'Boleto Compra Venta') {
    // pt-20 asegura que todo el texto baje lo suficiente para no superponerse con el logo
    printHtml += `
      <div class="pt-20">
