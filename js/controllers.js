...
window.toggleEstadoAuto = async (autoId) => {
  ...
  if (auto.estado === 'Vendido' || auto.estado === 'Señado') {
    return; 
  }
...
window.handleDAVentaSubmit = async (e, autoId) => {
  ...
  try {
    ...
    const vCredito = document.getElementById('chk-credito')?.checked 
      ? Number(document.getElementById('val-credito').value.replace(/[^0-9]/g, '')) 
      : 0;
      
    const cCredito = Number(document.getElementById('cuotas-credito')?.value || 0);
    
    const vPagare = document.getElementById('chk-pagare')?.checked 
      ? Number(document.getElementById('val-pagare').value.replace(/[^0-9]/g, '')) 
      : 0;
      
    const cPagare = Number(document.getElementById('cuotas-pagare')?.value || 0);
    
    const vPermuta = window.state.ventaData.tienePermuta 
    ...
    if (totalVentaOperacion <= 0) {
      alert("Debes especificar al menos una forma de pago válida o recibir una permuta para poder cerrar la venta.");
    ...
    console.error("Error en el cierre de la venta:", err);
  } finally {
    window.state.isSubmittingVenta = false;
    if (btn) {
      window.setBtnLoader(btn, false);
...
window.handleGlobalLeadSubmit = async (e) => { 
  ...
  try {
    ...
    console.error("Error al guardar lead global:", err);
  } finally {
...
window.handleDA_CRMSubmit = async (e, autoId) => { 
  ...
  try {
    ...
    console.error("Error al agregar lead desde el auto:", err);
  } finally {
...
window.handleCajaSubmit = async (e) => {
  ...
  try {
    ...
    const tipoTrans = document.getElementById('caja-tipo').value; 
    const desc = document.getElementById('caja-desc').value; 
    const categoriaTrans = document.getElementById('caja-cat').value; 
    const valorNum = Number(document.getElementById('caja-monto').value.replace(/[^0-9]/g, '')); 
    const autoIdAsoc = document.getElementById('caja-auto').value || null; 
    const compTipo = document.getElementById('caja-comprobante').value; 
    const compNum = document.getElementById('caja-comp-num').value; 
    let ivaRaw = document.getElementById('caja-iva').value;
    const ivaNum = ivaRaw ? Number(ivaRaw.replace(/[^0-9]/g, '')) : 0;
    ...
    if (autoIdAsoc && tipoTrans === 'gasto') { 
      ...
      await window.fbUpdate("autos", autoIdAsoc, { gastos: nuevosGastosAuto }); 
    }
...
window.cobrarCuotaVenta = async (ventaId, tipo) => {
  ...
  if (tipo === 'credito' && venta.credito) {
    ...
    const creditoActualizado = { ...venta.credito, pagadas: numeroDeCuota };
    datosDeActualizacion = { credito: creditoActualizado };
    
  } else if (tipo === 'pagare' && venta.pagare) {
    montoACobrar = venta.pagare.valorCuota;
    numeroDeCuota = venta.pagare.pagadas + 1;
    
    const pagareActualizado = { ...venta.pagare, pagadas: numeroDeCuota };
    datosDeActualizacion = { pagare: pagareActualizado };
  }
  ...
  if (montoACobrar > 0) {
    ...
    try {
      ...
      window.closeModal('modal-pendientes');
      alert(`La cuota #${numeroDeCuota} fue cobrada con éxito e ingresada a la caja chica.`);
      
    } catch(e) {
      console.error("Error al cobrar cuota:", e);
    } finally {
      if (btnDOM) {
...
window.handleComisionSubmit = async (e) => {
  ...
  try {
    ...
    if (!usrId || montoBono <= 0) {
      alert("Por favor, complete los datos correctamente. El monto debe ser mayor a 0.");
