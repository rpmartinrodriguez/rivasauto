...
window.handleCajaSubmit = async (e) => {
  ...
  try {
    ...
    const v = Number(document.getElementById('caja-monto').value.replace(/[^0-9]/g, '')); 
    ...
    const iva = ivaRaw ? Number(ivaRaw.replace(/[^0-9]/g, '')) : 0;
          ...
          ${window.state.autos.filter(a => a.estado !== 'Vendido').sort((a,b) => a.marca.localeCompare(b.marca) || a.modelo.localeCompare(b.modelo)).map(a => {
            ...
            return `
             <tr class="border-b border-black text-sm">
               <td class="p-2 border-r border-black font-bold uppercase">${a.marca} ${a.modelo} (${a.año})</td>
               <td class="p-2 border-r border-black uppercase">${a.patente}</td>
               <td class="p-2 border-r border-black uppercase">${a.color || '-'}</td>
               <td class="p-2 border-r border-black">${a.km || '-'}</td>
               <td class="p-2 border-r border-black uppercase text-[10px]">${a.condicion || '-'}</td>
               <td class="p-2 text-right font-bold whitespace-nowrap">${precioFmt}</td>
             </tr>
            `;
          }).join('')}
        </tbody>
...
window.handleDAVentaSubmit = async (e, autoId) => {
  ...
  try {
    ...
    const vEf = document.getElementById('chk-efectivo')?.checked ? Number(document.getElementById('val-efectivo').value.replace(/[^0-9]/g, '')) : 0;
    const vCr = document.getElementById('chk-credito')?.checked ? Number(document.getElementById('val-credito').value.replace(/[^0-9]/g, '')) : 0;
    const cCr = Number(document.getElementById('cuotas-credito')?.value || 0);
    const vPa = document.getElementById('chk-pagare')?.checked ? Number(document.getElementById('val-pagare').value.replace(/[^0-9]/g, '')) : 0;
...
window.handleComisionSubmit = async (e) => {
  ...
  try {
    ...
    const monto = Number(document.getElementById('comision-monto').value.replace(/[^0-9]/g, ''));
